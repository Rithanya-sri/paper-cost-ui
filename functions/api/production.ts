// Cloudflare Workers API for Production Records
// Handles CRUD operations for daily production data

// Type declaration for D1 Database
type D1Database = any;
...
export async function onRequestGet(context: { env: Env; params: any }) {
...
// PUT - Update record
export async function onRequestPut(context: { env: Env; request: Request; params: any }) {
    const { DB } = context.env;
    const { id } = context.params;
